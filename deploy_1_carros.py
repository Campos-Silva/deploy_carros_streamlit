...
if pagina == "Precificação de Veículos Usados":
    ...
    st.markdown('Essa página contêm o modelo em produção para "Precificação de Veículos Usados". Construí ele para ajudar pessoas, concessionárias e distribuidoras a obterem uma estimativa de preços de veículos baseados em suas caracteristicas.')
    ...
    st.image("https://raw.githubusercontent.com/Campos-Silva/Projeto_1_Precificacao_de_Veiculos_Usados/main/carro_venda_c.jpg")

    st.markdown('Esse modelo permite a tomada de decisões visando a maximização dos lucros e redução de perdas por meio da precificação adequada de veículos baseados em suas caractersticas. Assim, o modelo de Machine Learning sugerido aqui será importante para a tomada de decisões orientadas em dados, e por exemplo, poderá ser empregada tanto por agências revendedoras de veículos usados, como para pessoas que querem comprar ou vender veículos usados.')
    ...
    st.markdown('Utilize as variáveis abaixo para gerar o preço previsto do veículo baseado nessas caracteristicas')
