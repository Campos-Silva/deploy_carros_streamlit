...
if pagina == 'Custos de Seguro de Saúde':
    ...
    st.image('https://raw.githubusercontent.com/Campos-Silva/custos_seguro/main/imagem_2.jpg', width=500)
...
if pagina == 'Detecção de Fraudadores de Seguro':
    ...
    st.image('https://raw.githubusercontent.com/Campos-Silva/custos_seguro/main/imagem_5.png', width=500)

    st.markdown('Construí esse modelo preditivo para ajudar Agências de Seguro de Saúde a detectarem possíveis clientes que estejam fraudando informações e assim, obtendo valor de seguro abaixo do valor adequado as suas verdadeiras características, acarretando prejuízos econômicos a seguradora')

    st.markdown('A característica a ser detectada é se o cliente é fumante, quando na descrição havia informado que não era.')
    ...
    if st.button('EXECUTAR MODELO'):
        pred = predict_model(modelo_para_deteccao_de_fumantes, data = dados)['Label']
        st.write(pred)
        #saida = 'O valor predito é de ${:.2f}'.format(pred)
        #st.subheader(saida)
    ...
    st.image("https://raw.githubusercontent.com/Campos-Silva/custos_seguro/main/imagem_7.jpg")
